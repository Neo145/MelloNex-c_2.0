...
# ✅ Data Loading
data_file = "data/processed/Wpl_Processed_Stats_Advanced.csv"  # Update with actual path
try:
...
for col in missing_cols:
    df[col] = 0 if col not in ["team1", "team2", "last 5 matches", "venue"] else "Unknown"

if missing_cols:
    st.warning(f"⚠️ Missing columns: {', '.join(missing_cols)}. Default values applied.")

# ✅ JSON Parsing for Last 5 Matches
...
# ✅ Load AI Model
model_file = "src/api/models/wpl_winner_prediction.pkl"
try:
...
if stats.empty:
    ...
    total_matches = stats["total matches"].values[0]
    wins = stats["wins"].values[0]
    losses = stats["losses"].values[0]
    tied_matches = stats["tied matches"].values[0]
    ...
    form1 = stats["team1 recent form"].values[0]
    form2 = stats["team2 recent form"].values[0]
    st.write(f"**{team1} Recent Form:** {form1}  |  **{team2} Recent Form:** {form2}")

    # ✅ **Venue Performance**
    st.subheader("🏟️ Stadium Performance")
    if "venue" in df.columns and df["venue"].nunique() > 1:
        venue_chart = px.bar(df.groupby("venue")["wins"].sum().reset_index(), x="venue", y="wins", title="🏟️ Stadium Performance", color="wins")
        st.plotly_chart(venue_chart)
    else:
        st.info("No sufficient venue data available for analysis.")

    # ✅ **Toss Decision Analysis**
    toss_decision_counts = stats["toss_decision"].value_counts().to_dict()
    toss_chart = px.pie(
        names=["Chose to Bat", "Chose to Bowl"], 
        values=[toss_decision_counts.get("bat", 0), toss_decision_counts.get("bowl", 0)], 
    ...
    if not win_df.empty:
        ...
        win_chart = px.bar(win_df, x="team1", y="win_margin", color="winner", title="Win Margin Over Time", text="win_margin")
        st.plotly_chart(win_chart)
    else:
        st.warning("⚠️ No win margin data available.")

    # ✅ **Last 5 Matches Head-to-Head**
    st.subheader("📅 Last 5 Matches Head-to-Head")
    last_5_matches = stats["last 5 matches"].values[0]
    if isinstance(last_5_matches, pd.DataFrame) and not last_5_matches.empty:
